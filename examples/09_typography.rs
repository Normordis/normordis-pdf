...
fn main() {
    let out_dir = std::path::Path::new("examples/output");
    ...
    let pdf = DocumentBuilder::new("normordis-pdf v1.4.0 — Typography")
        ...
        .push(Paragraph::from_runs(
            vec![
                ...
                TextRun { text: ".".into(), ..Default::default() },
            ],
            TextAlign::Left,
            Some(11.0),
        ))
        ...
        .push(
            Paragraph::new("This paragraph has a light-blue background fill.")
                .background(RgbColor { r: 0.85, g: 0.92, b: 1.0 })
        ...
        .push(
            Paragraph::new("Background AND border — useful for callout boxes.")
                .background(RgbColor { r: 1.0, g: 0.97, b: 0.85 })
                .border(ParagraphBorder::box_border(
                    DecorationLine::with_color(RgbColor { r: 0.8, g: 0.6, b: 0.0 })
                ))
                .align(TextAlign::Left),
        )
        ...
        .push(Paragraph::new("Content before the section break. The next section starts on a new page."))
