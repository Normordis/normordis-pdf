# genpdf Analysis — Reference for normordis-pdf
...
### Formula (normordis-pdf, corrected)
...
normordis-pdf: **no kerning**. For typical institutional documents with body text
this is acceptable (kerning correction < 3% per word pair), but for display
fonts or large-size text it may be noticeable.

### Comparison summary

| Aspect | genpdf | normordis-pdf | Status |
|---|---|---|---|
| Advance width formula | `A/upm × glyph_h × fs × 25.4/72` | `A/upm × fs × 25.4/72` | ✅ normordis-pdf is more accurate |
...
| Justification | Caller's responsibility | Inline in `build_line` | ✅ normordis-pdf handles it correctly |
...
## 4. Recommended Patches for normordis-pdf
...
For normordis-pdf: the current fix (one BT/ET per segment) is correct and safe.
...
bold/italic per token across run boundaries. normordis-pdf's `(String, AppliedStyle)`
...
normordis-pdf would require changing the `Element::render` signature to return
