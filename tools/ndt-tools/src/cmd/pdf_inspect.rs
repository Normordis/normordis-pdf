...
pub fn print_report(summary: &PdfSizeSummary) {
    ...
    println!("{}", "─".repeat(39));
    println!("Fonts (embedded):     {:>8} KB  ({:.0}%)",
        summary.font_bytes / 1024, pct(summary.font_bytes, summary.total_bytes));
    println!("Content streams:      {:>8} KB  ({:.0}%)",
        summary.stream_bytes / 1024, pct(summary.stream_bytes, summary.total_bytes));
    println!("Images:               {:>8} KB  ({:.0}%)",
        summary.image_bytes / 1024, pct(summary.image_bytes, summary.total_bytes));
    println!("Metadata:             {:>8} KB  ({:.0}%)",
        summary.metadata_bytes / 1024, pct(summary.metadata_bytes, summary.total_bytes));
    println!("Other:                {:>8} KB  ({:.0}%)",
        summary.other_bytes / 1024, pct(summary.other_bytes, summary.total_bytes));
    println!("{}", "─".repeat(39));
    println!("Object count:         {:>8}", summary.object_count);
    println!("Compressed streams:   {:>8}", summary.compressed_streams);
    println!("Uncompressed streams: {:>8}  <- target: 0", summary.uncompressed_streams);
    ...
    if summary.font_bytes > 200_000 {
        println!("WARNING: Large font data ({} KB) — font subsetting needed (v2.0.0)",
...
enum ObjKind { Font, Image, Stream, Metadata, Other }
...
fn classify_object(obj: &Object) -> ObjKind {
    ...
    if let Ok(subtype) = dict.get(b"Subtype").and_then(|t| t.as_name()) {
        if subtype == b"Image" { return ObjKind::Image; }
    }
