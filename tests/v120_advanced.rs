use normordis_pdf::{
    BulletList, DocumentBuilder, Element, FixedBox, FixedTextBox, FontRegistry,
    OrderedList, OverflowPolicy, Paragraph, ParagraphContent, RenderResult,
    Spacer, Table, TableCell, TableRow, TextAlign, TextLayoutEngine, VerticalAlign,
    ListItemElement,
};
...
fn default_fonts() -> FontRegistry { FontRegistry::default() }
fn default_style() -> DocumentStyle { DocumentStyle::default() }

fn plain_rows(data: Vec<Vec<String>>) -> Vec<TableRow> {
    data.into_iter().map(|cells| TableRow::plain(cells)).collect()
}
...
fn table_50_rows_renders_without_panic() {
    let rows = plain_rows(
        (1..=50)
            .map(|i| vec![i.to_string(), format!("Item {}", i), format!("{:.2}", i as f64 * 1.5)])
            .collect(),
    );
...
fn bullet_list_100_items_renders_without_panic() {
    let items = plain_items(
        (1..=100).map(|i| format!("Item de lista número {}", i)).collect()
    );
...
fn ordered_list_60_items_produces_bytes() {
    let items = plain_items(
        (1..=60).map(|i| format!("Ponto {}", i)).collect()
    );
...
fn paragraph_estimated_height_grows_with_content() {
    ...
    assert!(long > short, "parágrafo longo deve ter altura estimada maior: {long} > {short}");
}
...
fn fixed_elements_with_z_index_render_without_panic() {
    let box_low = FixedTextBox {
        text_box: FixedBox { x_mm: 20.0, y_mm: 100.0, width_mm: 60.0, height_mm: 20.0,
            overflow: OverflowPolicy::Truncate, border: None, background: None,
            padding_mm: 2.0, z_index: 0, ua_role: None, ua_alt: None },
    ...
    let box_high = FixedTextBox {
        text_box: FixedBox { x_mm: 20.0, y_mm: 100.0, width_mm: 60.0, height_mm: 20.0,
            overflow: OverflowPolicy::Truncate, border: None, background: None,
            padding_mm: 2.0, z_index: 10, ua_role: None, ua_alt: None },
...
fn letter_spacing_increases_measured_width() {
    ...
    assert!(w1 > w0, "largura com letter_spacing deve ser maior: {w1} > {w0}");
}
...
fn letter_spacing_zero_is_same_as_default() {
    ...
    let run_zero = TR { text: "HELLO".into(), style: AppliedStyle::default(), letter_spacing_mm: 0.0, ..Default::default() };
...
fn paragraph_indent_left_renders_without_panic() {
    DocumentBuilder::new("Indentação")
        .push(
            Paragraph::new("Texto com indentação esquerda de 10mm.")
                .indent_left(10.0),
        )
        .render_to_bytes()
...
fn layout_right_short_line_has_positive_x_offset() {
    ...
    assert!(seg.x_offset_mm > 0.0,
        "linha curta alinhada à direita deve ter x_offset > 0: {}", seg.x_offset_mm);
}
...
fn layout_right_x_offset_near_max_minus_line_width() {
    ...
    assert!(diff < 0.5, "x_offset ({:.3}) deve ser próximo de max_w-line_w ({:.3})", seg.x_offset_mm, expected_offset);
}
...
fn layout_right_full_width_line_has_near_zero_offset() {
    ...
    let engine = TextLayoutEngine::new(&fonts, &style);
    let word = "M";
    let word_w = fonts.get_default().measure_text_mm(word, 11.0, false, false);
    let result = engine.layout_plain(
        &fonts,
        word,
        word_w + 0.01,
    ...
    let seg = &result.lines[0].segments[0];
    assert!(seg.x_offset_mm < 0.5,
        "linha que preenche a largura com Right deve ter x_offset ≈ 0: {}", seg.x_offset_mm);
}
